...
class ActivityNet_DataLoader(Dataset):
    def __init__(
        ...
        for root, dub_dir, video_files in os.walk(self.features_path):
            for video_file in video_files:
                ...
                video_id_ = video_id_[:-4]
                # if video_file == "v_BbQ39yhaC54.mkv":
                #     print(video_file)
                #     print(video_id_)
                # if video_id_ == "v_BbQ39yhaC54":
                #     print("true")
                #     print(video_id_)
                #     print(video_id_ in pseudo_video_id_list)
        ...
        # print(f"len(self.pseudeo_video_id_list):{len(self.pseudo_video_id_list)}")
        # print(f"len(self.video_id_list):{len(self.video_id_list)}")
        for pseudo_video_id, video_id in zip(self.pseudo_video_id_list, self.video_id_list):
            if pseudo_video_id not in self.pseudo_caption_dict or pseudo_video_id not in self.video_dict:
                continue
    ...
    def __getitem__(self, feature_idx):
        ...
        return pairs_text, pairs_mask, pairs_segment, video, video_mask, pseudo_video_id
