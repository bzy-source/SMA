...
class MSRVTT_DataLoader(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # #########    Test    #############
        # ##################################
        # frame_per_video = video.squeeze()
        # for idx in range(frame_per_video.shape[0]):
        #     frame = frame_per_video[idx]
        #     # 检查图像张量的维度
        #     print("Before permute:", frame.shape)  # 应该输出 torch.Size([3, 224, 224])

        #     # 如果发现张量的维度不对，调整顺序为 (C, H, W)
        #     # 如果形状已经是 (3, 224, 224)，则跳过这个步骤
        #     if frame.shape[0] == 224:  # 如果通道数在最后一维
        #         frame = frame.permute(2, 0, 1)  # 调整为 (C, H, W)

        #     print("After permute (if needed):", frame.shape)  # 确认为 torch.Size([3, 224, 224])
        #     frame_ = ToPILImage()(frame)
        #     # 指定保存文件夹和文件名
        #     save_folder = f'checkpoint/msrvtt_vit32/pool_video_sentence+select_frame_aggtrm/{video_id}'
        ...
        #     plt.imshow(frame_)
        #     plt.title(f"Video {video_id}, Frame {idx+1}")
        #     save_path = os.path.join(save_folder, file_name)
        #     plt.savefig(save_path, dpi=300, bbox_inches='tight')  # dpi指定分辨率, bbox_inches='tight'确保保存时没有多余空白
        ...
        # ##################################
        return pairs_text, pairs_mask, pairs_segment, video, video_mask, choice_video_ids[0]
