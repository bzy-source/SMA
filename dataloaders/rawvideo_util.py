    ...
    def video_to_tensor(self, video_file, preprocess, sample_fp=0, start_time=None, end_time=None):
        ...
        for sec in np.arange(start_sec, end_sec + 1):
            ...
            for ind in inds:
                ...
                ret, frame = cap.read()
                if not ret: break
                origin_frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)                 # [H, W, C] [224, 298, 3]
                frame_rgb = preprocess(Image.fromarray(origin_frame_rgb).convert("RGB"))  # [C, H, W] [3, 224, 224]
                images.append(frame_rgb)

                # frame_rgb_ = ToPILImage()(frame_rgb)
                # images_.append(frame_rgb_)
