...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(self, x: torch.Tensor):
        ...
        return self.attn(x, x, x, need_weights=True, attn_mask=attn_mask_)
...
class Transformer(nn.Module):
    ...
    # def forward(self, x: torch.Tensor):
    #     intermediate_features = []  # 用于存储每层的中间输出

    #     for i, block in enumerate(self.resblocks):
    #         x = block(x)  # 前向传播每一层
    #         if self._type=='t' and i==6:  # 如果需要返回中间特征
    #             intermediate_features.append(x)

    #     if self._type=='t':
    #         return x, intermediate_features  # 返回最终输出和所有中间层特征
    #     else:
    #         return x  # 如果不需要中间层特征，返回最终输出
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, return_hidden=False, video_frame=-1):
        hidden, attn = self.visual(image.type(self.dtype), video_frame=video_frame)
